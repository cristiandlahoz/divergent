...
struct AnnotationRenderContext<'a> {
    title: &'a str,
    items: &'a [String],
    selected: usize,
...
impl Modal {
    ...
    fn render_annotations(
        ...
        frame: &mut Frame,
        area: Rect,
        context: AnnotationRenderContext<'_>,
    ) {
        let t = theme::get();

        // Compact title with count
        let title_text = format!(" {} ({}) ", context.title, context.items.len());
        ...
        let (list_area, footer_area) = if context.export_input.is_some() {
            let chunks = Layout::default()
                .direction(Direction::Vertical)
                .constraints([
                    Constraint::Min(1),
        ...
        let scroll_offset = if context.selected >= visible_count {
            context.selected - visible_count + 1
        } else {
            0
        };

        let list_items: Vec<ListItem> = context
            .items
            .iter()
            .enumerate()
            .skip(scroll_offset)
            .take(visible_count)
            .map(|(i, item)| {
                let is_selected = i == context.selected;
        ...
        if let Some(input) = context.export_input {
        ...
        // Compact footer
        let footer_text = if context.export_input.is_some() {
            Line::from(vec![
