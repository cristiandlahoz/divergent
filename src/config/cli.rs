...
#[command(about = "Interactive terminal diff viewer for git and jj", long_about = None)]
#[command(version)]
pub struct Cli {
...
pub fn resolve_theme(cli_theme: Option<ThemeChoice>) -> Result<ThemeChoice, String> {
    ...
    match std::env::var("DIVERGENT_THEME") {
        ...
        Err(std::env::VarError::NotPresent) => Ok(ThemeChoice::Tokyonight),
...
mod tests {
    ...
    fn test_theme_cli_parses() {
        ...
        let Commands::Diff { theme, .. } = cli.command;
    ...
    #[test]
    fn test_theme_default_is_tokyonight() {
    ...
    fn test_theme_cli_overrides_env() {
        let guard = EnvGuard::set("DIVERGENT_THEME", "tokyonight");
        let cli_theme = Some(ThemeChoice::Midnight);
        let theme = resolve_theme(cli_theme).unwrap();
